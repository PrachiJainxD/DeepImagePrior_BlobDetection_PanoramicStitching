...
def detectBlobs(im, param=None):
    ...
    initSigma = 1.6 #Initial Scale
    factor = 1.25 #np.sqrt(2) #Factor by which scale is mulitplied each time  
    threshold =  0.0001
    finalSigma = np.power(factor, 15) #Final Scale
    ...
    if method == 0:
        ...
        #Downsampling image - Faster version
        # Ensure odd filter size
        filterSize = int(round(6 * sigmas[0]))
        # Ensure right - odd filter size   
        if filterSize % 2 == 0:
            filterSize += 1             
        LoG = LoGFilter(sigmas[0], filterSize )
        
        # Scale the image.
        for i in range(n):
            # Down scale
            scaledH = int(height * ((1/factor) ** i))
            scaledW = int(width * ((1/factor) ** i))
            scaledIm = transform.rescale(im, (1/factor) ** i, order=3)
            nextFilter= cv2.filter2D(scaledIm, cv2.CV_32F, LoG)**2
            # Upscale
            scaleSpace[:,:, i] = transform.resize(nextFilter, (height, width), order=3)
        ...
        DoG = LoGFilter(sigmas[0], filterSize) - LoGFilter(sigmas[0]*1.25, filterSize )
        ...
        for i in range(n):
            ...
            scaledW = int(width * ((1/factor) ** i))
            scaledIm = transform.rescale(im, (1/factor) ** i, order=3)
            nextFilter= cv2.filter2D(scaledIm, cv2.CV_32F, DoG)**2
            # Upscale
            scaleSpace[:,:, i] = transform.resize(nextFilter, (height, width), order=3)
    ...
    for level in range(n):#check >=
        ...
        radius = np.sqrt(2) * sigmas[level] * np.ones(len(centerX[level])) # Compute radius
